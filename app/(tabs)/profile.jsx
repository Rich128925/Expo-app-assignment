...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    auth.currentUser?.getIdToken(false).then(setToken).catch(() => {})
  }, [user])
      ...
      <View style={styles.container}>
        ...
        <View style={styles.userInfo}>
          ...
          <View style={styles.nameContainer}>
            ...
            <Typo style={styles.emailText}>
              {user?.email || 'No Email'}
            </Typo>

            {/* UID */}
            <View style={styles.infoRow}>
              <Typo size={12} color={'#737373'}>UID: </Typo>
              <Typo size={12} color={'#a3a3a3'} style={styles.infoValue}>
                {user?.uid || '—'}
              </Typo>
              <TouchableOpacity onPress={() => Alert.alert('UID', user?.uid || '')}>
            ...
            {token && (
              <View style={styles.infoRow}>
                <Typo size={12} color={'#737373'}>Token: </Typo>
                <Typo size={12} color={'#a3a3a3'} style={styles.infoValue}>
                  {token.slice(0, 20)}…
                </Typo>
                <TouchableOpacity onPress={() => Share.share({ message: token })}>
